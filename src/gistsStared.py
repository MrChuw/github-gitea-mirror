...
from src.helper import (
    get_config, gh_api, gitea_create_org, gitea_create_repo, gitea_create_user_or_org, gitea_get_user, gitea_set_repo_star,
    gitea_set_repo_topics,
)
...
def gists_stared():
    ...
    for repo in gh.get_user().get_starred_gists():
        ...
        logger.info('Gist : {0}/{1}'.format(repo.owner.login, repo.id))
        ...
        if status != 'failed':
            try:
                if status != 'exists':
                    ...
                    gitea_set_repo_star(repo_owner, m["repo_name"])
