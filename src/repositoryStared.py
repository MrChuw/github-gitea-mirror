...
from src.helper import (
    get_config, gh_api, gitea_create_repo, gitea_create_user_or_org, gitea_get_user, gitea_session, gitea_set_repo_star,
    gitea_set_repo_topics, is_blacklisted_repository,
)
...
def repositoryStared():
    ...
    for repo in gh.get_user().get_starred():
        logger.info('⭐  Star\'ed Repository : {0}'.format(repo.full_name))
        ...
        try:
            ...
            logger.info("[error] Github User / Repository Not Exists")
            continue
        ...
        if status != 'failed':
            try:
                if status != 'exists':
                    ...
                    gitea_set_repo_star(repo_owner, repo_name)
